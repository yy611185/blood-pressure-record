...
原型的假手机外框、状态栏、手势条、自绘数字键盘转为设备系统栏和原生数字 IME。录入按读数、情况、完成三步组织（原型与首轮实现的静坐引导页已在后续版本移除，只保留「测量前静坐 5 分钟」的文案提示），保留草稿、编辑、至少两组读数、平均策略、逐组校验、异常二次确认和高风险提示。原型演示至多 10 组；Android 领域规则 `MeasurementInputRules.MAX_READING_COUNT` 为 20 组，界面继续遵守既有 20 组上限。趋势继续使用已有 `RAW` / `DAILY` 聚合模式和真实数据库记录；原型图上的示例解释不改变计算规则。
