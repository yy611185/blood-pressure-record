...
本次提交完成了 Stitch 风格界面的增量落地，保持现有 Room / DataStore / Repository / ViewModel 业务逻辑不变。

### 已完成
- 首页“新增测量”改为卡片化输入，保留“前2组固定 + 第3组起动态扩展”。
- 展开后同时提供“收起第3组”和“添加下一组”按钮，新增组参与平均值/分级/高风险判断。
- 历史页改为分组列表 + 统计摘要 + 趋势入口；详情页支持查看、编辑、删除确认。
- 趋势页支持 7 天/30 天数据展示，并支持收缩压 / 舒张压 / 双曲线切换。
- 设置页保持一级菜单 + 二级页结构，用户资料、提醒、显示、数据管理、说明、免责声明全部可用。
- 全链路中文文案修复为可读状态（不改业务字段协议）。

### 保持不变
- 主数据存储：Room
- 设置存储：DataStore
- 导入导出：CSV / XLSX（应用私有目录 + 系统选择器入口）
- 导入/导出、录入/保存、历史编辑/删除核心逻辑
...
### 1.1.4（当前版本）
- 全量落地 Stitch 风格 UI（测量 / 历史 / 趋势 / 设置一级与二级页）。
- 修复当前生效页面中文乱码，统一中文文案。
- 保持现有业务架构与数据链路不变：Room、DataStore、Repository、ViewModel。
- 继续支持动态多组读数（第3组起可持续新增）并参与平均值/分级/高风险计算。
- 维持 3 栏底部导航（测量 / 历史 / 设置），二级页自动隐藏底栏。
- 导入导出与数据管理能力保持兼容（CSV / XLSX）。
