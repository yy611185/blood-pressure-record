        ...
        if (uiState.showTrendChart) {
            Card(
                ...
                Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                    Text("趋势图", style = MaterialTheme.typography.titleMedium, color = MaterialTheme.colorScheme.primary)
                    Text("查看最近 7 天与 30 天变化")
                }
            }
        }
                        ...
                        Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                                ...
                                Text(
                                    ...
                                    color = if (session.categoryText.contains("重度") || session.categoryText.contains("2期")) Color(0xFFB8141A) else MaterialTheme.colorScheme.primary
                                )
                            }
                            Text(
...
private fun EmptyStateCard() {
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
            ...
            Text("当前范围没有测量数据，先去“测量”页新增一条记录吧。")
        }
    }
}
