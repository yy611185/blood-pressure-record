...
fun SessionTimeSeriesDualLineChart(
    ...
    if (sessions.size == 1) {
        ...
        Text(
            "仅1条记录：${formatDateTime(one.measuredAt)}  收缩压 ${one.avgSystolic} / 舒张压 ${one.avgDiastolic} mmHg",
    ...
    Canvas(
        ...
        points.forEach { p ->
            val x = xOf(p.measuredAt)
            drawCircle(color = sysColor, radius = 4.8f, center = Offset(x, yOf(p.avgSystolic)))
            drawCircle(color = diaColor, radius = 4.8f, center = Offset(x, yOf(p.avgDiastolic)))
            drawLine(
        ...
        drawText(
            textMeasurer = textMeasurer,
            text = "收缩压",
            topLeft = Offset((xOf(last.measuredAt) + 8f).coerceAtMost(right - 40f), yOf(last.avgSystolic) - 16f),
            style = TextStyle(color = sysColor, fontSize = TextUnit(10f, TextUnitType.Sp))
        )
        drawText(
            textMeasurer = textMeasurer,
            text = "舒张压",
            topLeft = Offset((xOf(last.measuredAt) + 8f).coerceAtMost(right - 40f), yOf(last.avgDiastolic) - 16f),
        ...
        drawText(
            textMeasurer = textMeasurer,
            text = lastLabel,
            topLeft = Offset(right - 62f, bottom + 8f),
            style = TextStyle(color = Color(0xFF616A78), fontSize = TextUnit(9f, TextUnitType.Sp))
        )
