...
fun EditSessionScreen(
    ...
    Column(
        ...
        Text("编辑测量", style = MaterialTheme.typography.headlineMedium)
        if (uiState.loading) {
            Text("正在加载...")
            return@Column
        }
        Card(modifier = Modifier.fillMaxWidth()) {
            Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                if (uiState.message.isNotBlank()) {
                    val ok = uiState.message.contains("成功") || uiState.message.contains("保存")
...
private fun EditReadingBlock(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        OutlinedTextField(value = input.systolic, onValueChange = onSystolic, label = { Text("收缩压") }, modifier = Modifier.fillMaxWidth(), singleLine = true, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number))
        OutlinedTextField(value = input.diastolic, onValueChange = onDiastolic, label = { Text("舒张压") }, modifier = Modifier.fillMaxWidth(), singleLine = true, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number))
