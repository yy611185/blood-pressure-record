...
fun DoubleLineChart(sessions: List<SessionRecord>) {
    ...
    Canvas(
        ...
        val zZone = ZoneId.systemDefault()
        if (sorted.isNotEmpty()) {
            val firstDate = Instant.ofEpochMilli(sorted.first().measuredAt).atZone(zZone).format(formatter)
            val lastDate = Instant.ofEpochMilli(sorted.last().measuredAt).atZone(zZone).format(formatter)
            drawText(
                textMeasurer = textMeasurer,
                text = firstDate,
