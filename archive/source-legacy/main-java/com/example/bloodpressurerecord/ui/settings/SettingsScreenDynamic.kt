...
fun SettingsScreen(
    ...
    val menus = listOf(
        ...
        Triple("免责声明", "医学相关中性声明", onOpenDisclaimer)
    )
    Column(
...
fun SettingsInfoScreen(onBack: () -> Unit) {
    SettingsSubPageScaffold(title = "说明", onBack = onBack) {
        Text("版本 1.0.4 更新说明", style = MaterialTheme.typography.titleMedium)
        Text("1. 新增测量页支持动态扩展读数组：第3组起可连续添加第4组、第5组……")
        Text("2. 动态新增组会参与平均值、分级与高风险判断，保存/编辑链路已打通。")
        Text("3. 设置页改为一级菜单 + 二级详情页结构，分为用户资料、提醒、显示、数据管理、说明、免责声明。")
        Text("4. 数据导入导出能力保持兼容，CSV / XLSX 仍可使用。")
        Text("5. 本次为增量升级，保持既有稳定架构与核心功能。")
    }
}
