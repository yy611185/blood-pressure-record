...
fun HomeScreen(viewModel: HomeViewModel) {
    ...
    if (uiState.showHighRiskDialog) {
        AlertDialog(
            ...
            title = { Text("高优先级提醒") },
            text = { Text("检测到超过 180/120 的高值，请优先关注身体状态。是否继续保存？") },
    ...
    Column(
        ...
        Text("至少填写两组读数，分级由系统自动计算。", style = MaterialTheme.typography.bodyMedium)
        ...
        Card(modifier = Modifier.fillMaxWidth()) {
            Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                if (uiState.formMessage.isNotBlank()) {
                    val success = uiState.formMessage.contains("成功")
                    Text(
        ...
        Text("已保存会话数：$totalCount", style = MaterialTheme.typography.bodyMedium)
    }
}
