...
object AppReleaseNotes {
    val notes = listOf(
        ReleaseNote(
            ...
            summary = "完成趋势性能、数据可信度、提醒与备份迁移闭环，并整理工程发布流程。",
            changes = listOf(
                "趋势图改用真实时间轴、固定纵轴、共享降采样和每日聚合，改善长期数据卡顿与错位。",
                "Excel 备份 v2 无损保存任意组原始读数、场景和症状，并支持导回。",
                "血压分级和高风险阈值合并为唯一规则来源，高风险开关与保存时序修复。",
                "晨晚通知、大字号、趋势显示、事务清空和换机导入正式生效。",
                "清理重复源码和构建产物，Release 改为环境变量正式签名，不再回退 Debug 签名。",
                "版本更新至 v1.5.0。"
            )
        ),
        ReleaseNote(
