...
fun SettingsReminderScreen(viewModel: SettingsViewModel, onBack: () -> Unit) {
    ...
    val morningPermissionLauncher = rememberLauncherForActivityResult(
        ActivityResultContracts.RequestPermission()
    ) { granted ->
        if (granted) viewModel.setMorningReminderEnabled(true)
    }
    val eveningPermissionLauncher = rememberLauncherForActivityResult(
        ActivityResultContracts.RequestPermission()
    ) { granted ->
        if (granted) viewModel.setEveningReminderEnabled(true)
    }
    ...
    SettingsSubPageShell("提醒设置", onBack) {
        SettingsSwitchRow(
            ...
            if (enabled) enableWithPermission(morningPermissionLauncher) {
                viewModel.setMorningReminderEnabled(true)
            }
        ...
        SettingsSwitchRow(
            ...
            if (enabled) enableWithPermission(eveningPermissionLauncher) {
